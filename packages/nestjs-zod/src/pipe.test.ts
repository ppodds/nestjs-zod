...
testMany(
  'should use manually passed Schema / DTO for validation',
  async ({ z }) => {
...
describe('issue#162', () => {
  testMany(
    'should throw an error if strictSchemaDeclaration is true and using a primitive type',
    async () => {
  ...
  testMany(
    'should throw an error if strictSchemaDeclaration is true and using a class-validator DTO',
    async () => {
      const StrictZodValidationPipe = createZodValidationPipe({
        strictSchemaDeclaration: true,
      });
  ...
  testMany(
    'should not throw an error when using a primitive type if strictSchemaDeclaration is not set',
    async () => {
      @Controller('users')
      class UserController {
        @Post()
  ...
  testMany(
    'should not throw an error when using a class-validator DTO if strictSchemaDeclaration is not set',
    async () => {
      class ClassValidatorUserDto {
        @IsString()
        name!: string;
